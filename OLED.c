...
/* 亮度自动调节实现 */
uint8_t OLED_MapLightToContrast(uint16_t adc_val)
{
    // 限制ADC值范围，避免极端值影响
    if (adc_val < 100)
        adc_val = 100;
    ...
    // 映射ADC值到对比度 (50-255)
    // 光照越强(ADC值越小)，对比度越低
    return (uint8_t)( (adc_val  / 3950.0 ) *255);
}

void OLED_AutoAdjustBrightness(void)
{
    static uint8_t last_contrast = 0;
    uint16_t adc_val = HAL_ADC_GetValue(&hadc1);  
    uint8_t current_contrast = OLED_MapLightToContrast(adc_val);
    
    // 仅当对比度变化超过一定阈值时才更新，避免频繁调整
    if (abs(current_contrast - last_contrast) > 5)
    {
        OLED_SetContrastValue(current_contrast);
        last_contrast = current_contrast;
    }
}

// 手动设置亮度等级(0-10)
void OLED_ManualSetBrightness(uint8_t level)
{
//    if (level > 10) level = 10;
//    // 映射到50-255的对比度范围
//    uint8_t contrast = (uint8_t)(50 + (level * 205) / 10);
    OLED_SetContrastValue(level);
}
